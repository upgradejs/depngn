...
describe('validate', () => {
  it('throws the correct error when node version is invalid', () => {
    expect(() => validateArgs({ version: 'foo', reporter: Reporter.Terminal })).toThrow(
      `Invalid Node version: ${red('foo')}.`
    );
  });
  it('throws the correct error when reporter is invalid', () => {
    expect(() => validateArgs({ version: '1.0.0', reporter: 'foo' as Reporter.Terminal })).toThrow(
