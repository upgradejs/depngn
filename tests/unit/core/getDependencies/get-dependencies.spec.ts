...
const withPackageTestDir = 'tests/unit/core/getDependencies/mocks/withPackageJson';
const withoutPackageTestDir = 'tests/unit/core/getDependencies/mocks/withoutPackageJson';
...
describe('getDependencies', () => {
  ...
  it('reads dependencies from `package.json`', async () => {
    ...
    expect(output).toStrictEqual(['test-package-1', `test-package-2`, `test-package-3`]);
  });
  ...
  it('throws when no `package.json` is present', async () => {
    ...
    }).rejects.toEqual(new Error(`Unable to find package.json in ${process.cwd()}`));
  });
});
